...
String.prototype.toBase64 = function(){
    ...
    if (c > 0) { // Si el string NO es múltiplo de 3...
        for (; c < 3; c++) { // Añadimos ceros para que sea múltiplo de 3
            p += "="
            str = str + "\0"
        }
    }
    ...
    for (c = 0; c < str.length; c += 3) {
        ...
        var wordGroup = (str.charCodeAt(c) << 16) + (str.charCodeAt(c+1) << 8) + str.charCodeAt(c+2)
        console.log(wordGroup)
        // Y los separamos en 4 grupos de 6 bits (00111111 es una mascara!!)
        var mask = parseInt("00111111", 2)
        wordGroup = [(wordGroup >>> 18) & mask, (wordGroup >>> 12) & mask, (wordGroup >>> 6) & mask, wordGroup & mask]
        console.log(wordGroup)
        // Estos 4 elementos del array se usan de índices para sus caracteres del alfabeto
        r += base64chars[wordGroup[0]] + base64chars[wordGroup[1]] + base64chars[wordGroup[2]] + base64chars[wordGroup[3]]
        console.log(r)
    }
...
String.prototype.fromBase64 = function() {
    ...
    for (var c = 0; c < str.length; c += 4) {
        ...
        var wordGroup = (base64inv[str.charAt(c)] << 18) + (base64inv[str.charAt(c+1)] << 12) + (base64inv[str.charAt(c+2)] << 6) + base64inv[str.charAt(c+3)]
        console.log(wordGroup)
        ...
        r += String.fromCharCode((wordGroup >>> 16) & mask, (wordGroup >>> 8) & mask, wordGroup & mask)
        console.log(r)
    }
